...
public:
    Formula(std::string expr) : expr(expr) { UpdateData(); }
    ...
    std::vector<Position> GetReferencedCells() const { return ref_cells; }
            ...
            if (before <= cell.row) {
                ...
                while (pos != std::string::npos) {
                    ...
                    if (pos == std::string::npos)
                        break;
                    expr.replace(
                        expr.begin() + pos, expr.begin() + pos + old.ToString().size(),
                        cell.ToString()
                    );
                }
            }
